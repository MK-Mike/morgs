...
const mockTags: RouteTag[] = [
  ...
  { id: "9", name: "sustained", votes: 5, userVoted: false },
  { id: "10", name: "vertical", votes: 9, userVoted: false },
  { id: "11", name: "slabby", votes: 2, userVoted: false },
  { id: "12", name: "juggy", votes: 4, userVoted: false },
];
...
export default function RouteTags({ specifiedTags }: RouteTagCardProps) {
  ...
  }, [specifiedTags]); // Only re-run when specifiedTags changes
  ...
  const allTagNames = tags ? tags.map((tag) => tag.name) : [];

  // Sort tags by vote count (descending)
  const sortedTags = tags ? [...tags].sort((a, b) => b.votes - a.votes) : [];

  // If not initialized yet, show loading or empty state
  if (!isInitialized && specifiedTags && specifiedTags.length > 0) {
    return (
      <Card className="mx-auto w-full max-w-md">
        ...
        <CardContent>
          <div className="flex justify-center py-4">Loading tags...</div>
        </CardContent>
      </Card>
    );
  }
